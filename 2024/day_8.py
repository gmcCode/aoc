...
def antinodespart1(antennas, frequence):
    ...
    for larsA in range(1, len(antennas[frequence])):
        for larsB in range(larsA + 1, len(antennas[frequence])):
            deltax = (antennas[frequence][larsA][0] -
                      antennas[frequence][larsB][0])
            deltay = (antennas[frequence][larsA][1] -
                      antennas[frequence][larsB][1])
            pos1 = (antennas[frequence][larsA][0] - deltax,
                    antennas[frequence][larsA][1] - deltay)
            if (pos1[0] == antennas[frequence][larsB][0] and pos1[1] == antennas[frequence][larsB][1]):
                pos1 = (antennas[frequence][larsA][0] + deltax,
                        antennas[frequence][larsA][1] + deltay)

            pos2 = (antennas[frequence][larsB][0] - deltax,
                    antennas[frequence][larsB][1] - deltay)
    ...
    for larsA in range(1, len(antennas[frequence])):
        for larsB in range(larsA + 1, len(antennas[frequence])):
            ...
            while pos[0] <= len(input[0])-1 and pos[0] >= 0 and pos[1] <= len(input)-1 and pos[1] >= 0:
                antinodes.add(pos)
                pos = (antennas[frequence][larsA][0] - laufLars * deltax,
                       antennas[frequence][larsA][1] - laufLars * deltay)
                laufLars += 1
            laufLars = 1
            pos = antennas[frequence][larsA]
            while pos[0] <= len(input[0])-1 and pos[0] >= 0 and pos[1] <= len(input)-1 and pos[1] >= 0:
